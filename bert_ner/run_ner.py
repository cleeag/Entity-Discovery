...
def main():
    ...
    parser.add_argument("--output_dir",
                        default=f"./outputs/output_{datetime.now().strftime('%m%d_%I%M')}",
                        type=str,
                        # required=True,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--latest_dir",
                        default=f"./outputs/latest",
                        type=str,
                        # required=True,
                        help="The output directory where the latest model checkpoints will be written.")
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            eval(args, processor, label_list, tokenizer, device, model)
        ...
        json.dump(model_config, open(os.path.join(args.latest_dir, "model_config.json"), "w"))
        # Load a trained model and config that you have fine-tuned
    else:
        output_config_file = os.path.join(args.latest_dir, CONFIG_NAME)
        output_model_file = os.path.join(args.latest_dir, WEIGHTS_NAME)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        logger.info("\n%s", report)
        output_eval_file = os.path.join(args.latest_dir, "eval_results.txt")
